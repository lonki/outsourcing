...
export function touchDeviceHoverHandlerByClass(target, className) {
  const classAry = className.split(' ');

  for (let i = 0; i < target.length; i += 1) {
    addMultipleListeners(target[i], ['mouseover', 'touchstart'], () => {
      target[i].classList.add(...classAry);
    });

    addMultipleListeners(target[i], ['mouseout', 'touchend'], () => {
      target[i].classList.remove(...classAry);
    });
  }
}